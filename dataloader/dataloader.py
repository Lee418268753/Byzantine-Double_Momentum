...
class iidLoader(customDataLoader):
    ...
    def getPartitions(self):
        ...
        partition_list = []
        for i, frac in enumerate(partition_sizes):
            part_len = int(frac * data_len)
            if i == self.size - 1: 
                partition_list.append(indexes)
            else:
...
class dirichletLoader(customDataLoader):
    ...
    def getPartitions(self):
        ...
        if self.size*niid_data_per_device < len(self.dataset):
            ...
            extra_data_idxs = idxs[self.size * remaining_data_per_device:]
            for i, idx in enumerate(extra_data_idxs):
                users_idxs[i % self.size].append(idx)
        return users_idxs
